...
const RelatedVideos = ({ videos }: VideoProps) => {
  return (
    <div className="py-8">
      <h2  className="pb-6 uppercase text-[1.8rem] sm:text-[2.5rem] font-bold"> related Videos</h2>
      <div className="grid  grid-cols-2 gap-x-4 sm:grid-cols-3 gap-y-8 sm:gap-x-8 ">
        {videos.slice(0, 6).map((video) => (
          <div className="video" key={video.id}>
            ...
            ></iframe>
            <h3 className="mt-4 text-base sm:text-[1.3rem] font-semibold text-center"> {video.name}</h3>
          </div>
        ))}
      </div>
